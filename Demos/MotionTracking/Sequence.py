...
class Sequence:
    def __init__(self):
        ...
        self.RootTrajectory = None
        self.Motion = None
        self.Contacts = None
        self.BoneNameToIndex = None

    def SampleRoot(self, timestamp: float):
        a, b, w = self.GetIndexPair(timestamp)
        return Transform.Interpolate(
            self.RootTrajectory.Transforms[a], self.RootTrajectory.Transforms[b], w
        )
    ...
    def Draw(self, actor, bones, color=None):
        for i in range(self.Timestamps.size):
            AI4Animation.Draw.Skeleton(
                self.RootTrajectory.Transforms[i],
